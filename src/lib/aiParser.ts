...
4. AGENCY NAME (代理店) & TARGET SHEET: You MUST extract the advertising agency (e.g., 電通, 博報堂, etc.) for EACH file independently. If a file does not explicitly contain the agency name, DO NOT copy the agency name from a different file. If you cannot confidently determine the agency for a specific file, leave "代理店" blank "" and set "targetSheet" to "アザー見積" (Other). If it's Dentsu, targetSheet is "電通見積". If Hakuhodo, targetSheet is "博報堂見積". SPECIAL RULE FOR HAKUHODO: If it is a Hakuhodo estimate, you MUST set the "代理店" field strictly to one of the following based on keywords found in the estimate: "博報堂（博報堂）" (if you find 博報堂), "博報堂（大広）" (if 大広), "博報堂（読広）" (if 読広), "博報堂（朝広）" (if 朝広), or "博報堂（MP）" (if MP).
